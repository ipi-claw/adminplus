...
# AdminPlus 前后端联调测试脚本
# 测试范围：登录、用户管理、角色管理、权限管理、菜单管理、字典管理、配置管理、日志管理
...
LOGIN_RESPONSE=$(test_api "用户登录" "POST" "/v1/auth/login" "$LOGIN_DATA" "200" "false")
TOKEN=$(echo $LOGIN_RESPONSE | jq -r '.data.accessToken')
log_info "Token: $TOKEN"

# 1.3 登录测试 - 错误的用户名密码
log_info "1.3 登录测试 - 错误的用户名密码"
LOGIN_DATA_ERROR='{
    "username": "admin",
...
test_api "用户登录-错误密码" "POST" "/v1/auth/login" "$LOGIN_DATA_ERROR" "401" "false"
