...
# 尝试从响应中提取 Token
TOKEN=$(echo "$LOGIN_RESPONSE" | tail -5 | jq -r '.data.token' 2>/dev/null)
...
# 获取新的验证码
NEW_CAPTCHA_RESPONSE=$(curl -s "$BASE_URL/v1/captcha")
