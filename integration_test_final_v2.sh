...
echo "=========================================="
echo "  AdminPlus 前后端联调测试 - 最终版本 V2"
echo "=========================================="
echo ""
...
elif [ -z "$UNAUTH_CODE" ]; then
    log_warning "无权限访问用户列表 (返回空响应，可能是 CSRF 保护)"
else
...
CREATE_MENU_DATA="{
    \"parentId\": 2,
    \"type\": 1,
    \"name\": \"测试菜单${TIMESTAMP2}\",
    \"path\": \"/test${TIMESTAMP2}\",
    \"component\": \"test${TIMESTAMP2}/index\",
    \"icon\": \"test\",
    \"sortOrder\": 100,
    \"visible\": 1,
    \"status\": 1
...
TIMESTAMP3=$(date +%s)
CREATE_DICT_DATA="{
    \"dictType\": \"test_dict_${TIMESTAMP3}\",
    \"dictName\": \"测试字典${TIMESTAMP3}\",
...
if [ -n "$DICT_ID" ] && [ "$DICT_ID" != "null" ]; then
    ...
    # 6.4 根据字典类型查询
    test_api "根据字典类型查询" "GET" "/v1/sys/dicts/type/test_dict_${TIMESTAMP3}" "" "200" "true"

    # 6.5 更新字典
    UPDATE_DICT_DATA="{
        \"dictName\": \"测试字典${TIMESTAMP3}-更新\",
...
if [ -n "$INFO_RESPONSE" ] && [ "$INFO_RESPONSE" != "null" ] && [ "$INFO_RESPONSE" != "" ]; then
    log_success "应用信息"
else
    log_warning "应用信息 (返回空响应)"
fi
